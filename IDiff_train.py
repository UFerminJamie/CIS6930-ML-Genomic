...
class Trainer:
    ...
    def _run_epoch(self, epoch):
        ...
        for x, y, x_coord, y_coord in self.train_data:
            x = x.unsqueeze(1).to(self.gpu_id)  # (N, 1, NumGene)
            y = y.to(self.gpu_id)               # (N, NumEmbed)
            x_coord = x_coord.to(self.gpu_id)  
            y_coord = y_coord.to(self.gpu_id)
...
def assemble_dataset(input_args):
    ...
    all_count_mtx_df = pd.DataFrame(all_count_mtx_ori, columns=selected_genes, index=list(range(all_count_mtx_ori.shape[0])))
    ...
    all_count_mtx_ori = np.array(all_count_mtx_df.loc[spot_idx_to_keep, :])
    all_img_ebd_ori = all_img_ebd_ori[spot_idx_to_keep, :]
    all_img_ebd_aug = all_img_ebd_aug[spot_idx_to_keep, :]
    all_coord_ori = all_coord_ori[spot_idx_to_keep, :]
    input_args.logger.info(f"After exclude rows with all nan/zeros: {all_count_mtx_ori.shape}, {all_img_ebd_ori.shape}, {all_img_ebd_aug.shape}, {all_coord_ori.shape}")
    ...
    all_coord = torch.from_numpy(np.concatenate((all_coord_ori, all_coord_aug), axis=0))
    ...
    all_count_mtx_selected_genes = np.log2(all_count_mtx_df + 1).copy()
    ...
    alldataset = CustomDataset(torch.from_numpy(all_count_mtx_selected_genes.values).float(), 
                               all_img_ebd.float(), all_coord.long())    
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="./hestk_datasets/kidney/", help="Dataset path")
    parser.add_argument("--results_dir", type=str, default="./kidney_results/runs/", help="Path to hold runs")
    ...
    parser.add_argument("--model", type=str, default="IDiff")
